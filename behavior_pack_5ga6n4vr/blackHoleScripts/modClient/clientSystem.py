...
class BlackHoleClientSystem(ClientSystem):
	...
	def ListenEvent(self):
		...
		self.ListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.CreateEffectEvent, self,
							self.OnCreateEffect)
		self.ListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.SetSfxScaleEvent, self,
							self.OnSetSfxScale)
		self.ListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.ShowDeleteButtonEvent, self,
							self.OnShowDeleteButton)
		self.ListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.RemoveButtonUiEvent, self,
							self.OnRemoveButtonUi)
		self.ListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.PlayerAboutEvent, self,
							self.OnPlayerAbout)
		self.ListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.CloseAttractSwitchEvent, self,
							self.OnCloseAttractSwitch)

	def UnListenEvent(self):
		self.UnListenForEvent(clientApi.GetEngineNamespace(), clientApi.GetEngineSystemName(),
							  modConfig.OnScriptTickClient, self, self.OnScriptTickClient)
		self.UnListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.CreateEffectEvent, self,
							  self.OnCreateEffect)
		self.UnListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.SetSfxScaleEvent, self,
							  self.OnSetSfxScale)
		self.UnListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.ShowDeleteButtonEvent, self,
							  self.OnShowDeleteButton)
		self.UnListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.RemoveButtonUiEvent, self,
							  self.OnRemoveButtonUi)
		self.UnListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.PlayerAboutEvent, self,
							  self.OnPlayerAbout)
		self.UnListenForEvent(modConfig.ModName, modConfig.ModServerSystemName, modConfig.CloseAttractSwitchEvent, self,
							  self.OnCloseAttractSwitch)
	...
	def AttractPlayer(self):
		...
		if self.ar and self.x and self.y and self.z:
			...
			# 设置给玩家的移动向量大小
			pos_p = (
				float(self.x - playerPosX) / 200, float(self.y - playerPosY) / 200, float(self.z - playerPosZ) / 200)
