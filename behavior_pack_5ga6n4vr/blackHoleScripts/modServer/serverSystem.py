...
class BlackHoleServerSystem(ServerSystem):
    ...
    def OnServerBlockUseEvent(self, args):
        ...
        self.flag = True
        dict['playerId'] = args["playerId"]
        dict['x'] = args["x"]
        dict['y'] = args["y"]
        dict['z'] = args["z"]
        print '==========================> playerId = ', dict.get('playerId')
        print '==========================> x = ', dict.get('x')
        print '==========================> y = ', dict.get('y')
        print '==========================> z = ', dict.get('z')
    ...
    def OnScriptTickServer(self):
        ...
        if self.count % 30 == 0:
            print '-------------------------------------------------- tick'
            # 黑洞初始特效创建完成后，调用自定义函数，初步实现以玩家为中心，一定范围内的生物牵引功能
            # if self.flag:
            #     self.biologyAttract(dict.get('playerId'), dict.get('x'), dict.get('y'), dict.get('z'))
            #     print '-------------------------------------------------- biologyAttract'

    # 初步实现以玩家为中心，一定范围内的吸引功能
    def biologyAttract(self, player_id, x, y, z):
        entity_ids = get_entity_api.get_pet_enemies_around(player_id, 9)
        # print '------------------------------ entity_ids = ', entity_ids
        print '-----------------------------> count : ', len(entity_ids)
        for id in entity_ids:
            # print '================= id = ', id
            # 获取实体位置坐标
            comp = serverApi.GetEngineCompFactory().CreatePos(id)
            ...
            entityPosY = entityPos[1]
            entityPosZ = entityPos[2]
            set_motion(id, (float(x - entityPosX) / 10, float(y - entityPosY) / 10, float(z - entityPosZ) / 10))
        print '========================== biologyAttract  over'
